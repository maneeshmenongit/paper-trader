# Paper Trader — System Architecture (v1)

**Document ID:** `PAPER_TRADER_ARCH_001`
**Author:** Claude (Technical Architect role)
**Date:** April 2026
**Status:** Draft for review — ready for handoff to Product Manager role
**Depends on:** `PAPER_TRADER_HANDOFF_001.md` (scope, validated patterns to reuse, zero-cost stack)
**Lineage:** Sibling of `ORACLE_AGENTS_ARCH_001`. Reuses validated infrastructure patterns from oracle-agents by direct file copy; the domain layer, data interfaces, supervisor routing decisions, and scoring math are new.
...
5. **Symmetric prediction tracking, asymmetric execution.** The Predict agent emits UP/DOWN/HOLD with magnitude and confidence for every asset. The Execute agent acts on UP only in v1 (LONG-only with HOLD-as-no-trade); DOWN and HOLD predictions are recorded as `TradeDecision(executed=False)` so the post-mortem layer can score the prediction even though no money moved. SHORT support is deferred to Phase 2.
...
**Predict.** For each asset with a research bundle, one Gemini call using the **humble prompt template** (`prompts/predict_humble.py`). Returns `DirectionalPrediction` with direction (UP/DOWN/HOLD), confidence (0-1), magnitude_pct (expected % move), time_horizon_hours (default 24, configurable), and reasoning. The baseline prediction is computed in parallel with no LLM call — pure technical indicator: "if yesterday's close was higher than two days ago's close, predict UP, else DOWN." Both go into state.
